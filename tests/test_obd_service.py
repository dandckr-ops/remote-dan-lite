...
class MismatchedConfirmedDtcCountProvider(SimulatorOBDProvider):
    def query(self, command: str) -> str:
        canonical = command.replace(" ", "").upper()
        if canonical == "0101":
            return "7E8 06 41 01 02 07 E5 00\r>"
        return super().query(canonical)


class MissingStoredDtcResponseProvider(SimulatorOBDProvider):
    def query(self, command: str) -> str:
        canonical = command.replace(" ", "").upper()
        if canonical == "03":
            return "NO DATA\r>"
...
def test_obd_service_marks_stored_results_partial_when_pid_0101_count_disagrees(
    tmp_path: Path,
) -> None:
    database = EvidenceDatabase(tmp_path / "remote-dan.sqlite3")
    database.initialize()
    service = OBDService(
        database=database,
        simulator_provider=MismatchedConfirmedDtcCountProvider(),
    )
    service.connect(mode="simulator", session_id=None)

    faults = service.read_faults()

    assert faults["readiness"][0]["dtc_count"] == 2
    assert len(faults["stored"]) == 3
    assert faults["stored_status"] == "partial"
    assert any(
        item.get("command") == "03"
        and item.get("ecu") == "7E8"
        and "reported 2" in item.get("error", "")
        and "decoded 3" in item.get("error", "")
        for item in faults["errors"]
    )


def test_obd_service_challenges_no_data_when_pid_0101_reports_confirmed_dtcs(
    tmp_path: Path,
) -> None:
    database = EvidenceDatabase(tmp_path / "remote-dan.sqlite3")
    database.initialize()
    service = OBDService(
        database=database,
        simulator_provider=MissingStoredDtcResponseProvider(),
    )
    service.connect(mode="simulator", session_id=None)
    ...
    assert faults["readiness"][0]["dtc_count"] == 3
    assert faults["stored"] == []
    assert faults["stored_status"] == "partial"
    assert any(
        item.get("command") == "03"
        and item.get("ecu") == "7E8"
        and "reported 3" in item.get("error", "")
        and "decoded 0" in item.get("error", "")
        for item in faults["errors"]
    )
