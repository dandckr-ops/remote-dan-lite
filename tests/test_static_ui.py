...
def test_load_bank_mobile_layout_contains_intrinsic_width_and_reveals_active_tab() -> None:
    ...
    assert "/static/app.css?v=can-v2-obd-loadbank-0.4.0" in markup
    assert "/static/app.js?v=can-v2-obd-loadbank-0.4.0" in markup
...
def test_can_tab_exposes_accessible_bounded_existing_capture_decode_contract() -> None:
    ...
    assert "can-v2-obd-loadbank-0.4.0" in index
...
def test_can_analysis_v2_ui_exposes_diagnostics_heatmap_timeline_and_comparison_safely() -> None:
    ...
    assert "CanUiCompat.identifierMetrics" in script
...
def test_obd_browser_code_fences_reads_recovers_polling_and_has_uuid_fallback() -> None:
    ...
    assert "const obdRecordsRequestGate = CanRequestGate.createLatestRequestGate();" in script
    records_loader = script.split("async function loadObdRecords()", 1)[1].split(
        "function bindObdControls", 1
    )[0]
    assert "obdRecordsRequestGate.begin()" in records_loader
    assert "obdRecordsRequestGate.isCurrent" in records_loader
    assert "const obdSessionMutationGate = CanRequestGate.createLatestRequestGate();" in script
    session_submit = script.split('$("#obd-session-form").addEventListener', 1)[1].split(
        "\n  });", 1
    )[0]
    assert "obdSessionMutationGate.begin()" in session_submit
    assert session_submit.count("obdSessionMutationGate.isCurrent") >= 3


def test_load_bank_status_polling_is_latest_wins() -> None:
    script = (STATIC_DIR / "app.js").read_text(encoding="utf-8")
    assert "const loadBankStatusRequestGate = CanRequestGate.createLatestRequestGate();" in script
    loader = script.split("async function loadLoadBankStatus", 1)[1].split(
        "async function discoverLoadBankControllers", 1
    )[0]
    assert "loadBankStatusRequestGate.begin()" in loader
    assert "loadBankStatusRequestGate.isCurrent" in loader
...
def test_changed_static_assets_use_combined_release_cache_key() -> None:
    ...
    assert "/static/app.css?v=can-v2-obd-loadbank-0.4.0" in markup
    assert "/static/app.js?v=can-v2-obd-loadbank-0.4.0" in markup
    assert "/static/can_request_gate.js?v=can-v2-obd-loadbank-0.4.0" in markup
    assert "/static/can_ui_compat.js?v=can-v2-obd-loadbank-0.4.0" in markup
