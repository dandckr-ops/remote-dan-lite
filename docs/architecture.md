...
The OBD lane is intentionally separate from passive CAN acquisition. It actively sends bounded SAE J1979 requests through one long-lived, serialized provider while preserving the source ECU and exact adapter transcript. The operator surface is hardware-only and uses imperial normalized values (`°F`, `mph`, `psi`, and `lb/min`); deterministic simulators remain internal test and activation fixtures and are never selectable as devices. The first hardware provider opens only the configured `/dev/serial/by-id/...` OBDLink SX path at 115200 baud, requests exclusive tty ownership, holds a process-external lock, and is deliberately pinned to the commissioned ISO 15765-4 CAN 11/500 transport. Other J1979 transports are not yet claimed. It does not silently fall back from hardware to simulator.

Supported-PID discovery starts at Mode 01 PID 00 and follows continuation pages only when advertised. Initial normalized reads cover readiness/MIL, load, temperatures, fuel trims, MAP, RPM, speed, timing, MAF, throttle, runtime, fuel level, and module voltage. DTC reads retain stored, pending, and permanent classifications separately; the operator view must show each service as complete, partial, unavailable/no-data, or failed rather than displaying unavailable as a zero count. Mode 09 PID 02 VIN observations retain the reporting ECU and surface mismatches rather than choosing silently. Until additional Mode 09 PIDs are implemented, that surface is labeled Vehicle ID and must not present adapter, protocol, or responder connection metadata as vehicle facts.
